...
public class CambridgeTemplateProvider implements ViewProcessor<String> {
	...
	@Inject
	public CambridgeTemplateProvider(ServletContext context) {
		setExpressionLanguage();

		File templateDir = new File(context.getRealPath(TEMPLATE_DIR));
		this.loader = new DirectoryTemplateLoader(templateDir, UTF_8, checkInterval());
	}
	...
	public void writeTo(String t, Viewable viewable, OutputStream out) throws IOException {
		...
		if (isProd()) {
			fact = factories.get(t);
			if (fact == null) {
				fact = loader.newTemplateFactory(t);
				factories.put(t, fact);
			}
		} else {
			fact = loader.newTemplateFactory(t);
		}
